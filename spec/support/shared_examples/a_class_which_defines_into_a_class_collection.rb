...
RSpec.shared_examples_for "a class which defines into a class collection" do |method, collection|
  subject(:define) { example_state_class.__send__(method, value) }
  ...
  describe "defines value" do
    ...
    shared_examples_for "an value is defined" do
      it "adds to _values" do
        expect { define }.to change { example_state_class.public_send(collection) }.from([]).to([ value ])
      end
    end

    context "when no block is given" do
      subject(:define) { example_state_class.__send__(method, value, default: default) }

      it_behaves_like "an value is defined"

      it "defines an static default" do
        define
        expect(example_state_class).to have_received(:define_default).with(value, static: default)
      end
    end

    context "when a block is given" do
      subject(:define) { example_state_class.__send__(method, value, default: default, &block) }
      ...
      shared_examples_for "values are handed off to define_default" do
        it "calls define_default" do
          define
          expect(example_state_class).to have_received(:define_default).with(value, static: default, &block)
        end
      end
  ...
  it "defines an attribute" do
    define
    expect(example_state_class).to have_received(:define_attribute).with(value)
  end
end
